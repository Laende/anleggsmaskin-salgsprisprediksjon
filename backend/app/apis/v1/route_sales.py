...
@router.post(
    "/create-sale", response_model=SalesShow, name="Create sale", status_code=200
)
...
@router.get(
    "/all/{limit}/{order_by}",
    response_model=List[SalesShow],
    name="List sales",
    status_code=200,
)
async def retrieve_sales(
    limit: int, order_by: str = "saledate", db: Session = Depends(get_db)
):
    MAX_LIMIT = 100
    if limit >= MAX_LIMIT:
        limit = 100
    ...
    if not sales:
        raise HTTPException(
            ...
            detail=f"Sales ordered by {order_by} do not exist",
        )
    return sales
...
async def retrieve_sales_by_new_filter(is_new: bool, db: Session = Depends(get_db)):
    ...
    if not sales:
        message = (
            f"No new sales are added yet, try doing some predictions and try again."
        )
...
async def get_sale(id: int, db: Session = Depends(get_db)):
    ...
    if not sale:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Sale with id {id} does not exist.",
        )
    return sale
