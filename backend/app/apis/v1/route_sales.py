...
async def create_sale(sale: SalesCreate, db: Session = Depends(get_db)) -> Sales:
    sale: Sales = await create_new_sales(sale=sale, db=db)
    return sale


@router.get("/all/{limit}",response_model=List[SalesShow],name="List sales",status_code=200)
async def retrieve_sales(limit: int, db: Session = Depends(get_db)):
    sales = await list_sales(limit=limit, db=db)
    if not sales:
...
async def retrieve_sales_by_new_filter(is_new: bool, db: Session = Depends(get_db)):
    sales = await retrieve_sales_by_new(is_new=is_new, db=db)
    if not sales:
...
async def get_sale(id: int, db: Session = Depends(get_db)):
    sale = await get_sale_by_id(id=id, db=db)
    if not sale:
...
async def delete_sale(id: int, authenticated: bool = Depends(authenticate), db: Session = Depends(get_db)):
    if not authenticated:
        raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,detail=f"You are not authorized.",)
    sales = await delete_sales_by_id(id=id, db=db)
    if not sales:
